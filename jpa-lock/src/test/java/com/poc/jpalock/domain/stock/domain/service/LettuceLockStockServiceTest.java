...
@SpringBootTest
@DisplayName("재고 관리 기능 테스트(redis락)")
class LettuceLockStockServiceTest {
    ...
    class decrease_메소드는 {
        ...
        @Test
        void decreaseWithSpinLock_메서드는_재고_감소가_잘_진행된다() throws InterruptedException {
            ...
            for (int i = 0; i < threadCounts; i++) {
                executorService.submit(() -> {
                    try {
                        lettuceLockStockService.decreaseWithSpinLock(new StockCommand(1L, 1L, 1L));
